...
export type DepthCarouselItemInput = string | DepthCarouselItem

export interface DepthCarouselProps {
  items?: DepthCarouselItemInput[]
  cardWidth?: number
  cardHeight?: number
  radius?: number
  tint?: string
  ...
  onItemClick?: (item: DepthCarouselItem, index: number) => void
  className?: string
}
...
const normalizeItem = (it: DepthCarouselItemInput): DepthCarouselItem =>
...
export default function DepthCarousel({
  ...
  const onCardClick = useCallback(
    (index: number) => {
      ...
      if (active === index && onItemClick) {
        onItemClick(data[index], index)
      } else {
        setFocus(index, true)
      }
    },
    [active, data, onItemClick, setFocus]
  )
          ...
          >
            ...
            {(item.title || item.category) && (
              <div className="depth-carousel__card-overlay">
                {item.category && <span className="depth-carousel__card-badge">{item.category}</span>}
                {item.title && <h4 className="depth-carousel__card-title">{item.title}</h4>}
                {active === i && (
                  <span className="depth-carousel__card-action">
                    {item.actionText || "Click to explore →"}
                  </span>
                )}
              </div>
            )}
